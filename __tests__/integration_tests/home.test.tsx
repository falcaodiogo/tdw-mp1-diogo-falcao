...
jest.mock("../../app/cover-image", () => ({
  __esModule: true,
  default: jest.fn(() => (
    <div data-testid="cover-image">Cover Image Component</div>
  )),
}));
...
describe("HomePage", () => {
  ...
  test("renders intro section with correct content", async () => {
    ...
    expect(
      screen.getByText(/Aplicação de exemplo para o MP1 com:/),
    ).toBeInTheDocument();
  ...
  test("renders hero post when posts exist", async () => {
    ...
    expect(
      screen.getByText("This is the gemini post excerpt"),
    ).toBeInTheDocument();
  ...
  test("renders hero post link with correct href", async () => {
    ...
    const heroPostLink = screen.getByRole("link", {
      name: "Gemini Post Title",
    });
